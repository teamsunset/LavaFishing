...
class NodeEvaluatorLavaAmphibious(private val prefersShallowSwimming: Boolean) : WalkNodeEvaluator() {
    ...
    override fun done() {
        // 恢复可行走地形的原始路径寻找代价。
        mob.setPathfindingMalus(PathType.WALKABLE, this.oldWalkableCost)
        // 恢复水边的原始路径寻找代价。
        mob.setPathfindingMalus(PathType.WATER_BORDER, this.oldWaterBorderCost)
        super.done()
    }
    ...
    override fun getTarget(x: Double, y: Double, z: Double): Target = this.getTargetNodeAt(x, y + 0.5, z)
    ...
    override fun getNeighbors(outputNodes: Array<Node>, currentNode: Node): Int {
        ...
        // 获取当前节点上方、当前平面的方块类型
        val blockAbove = this.getCachedPathType(currentNode.x, currentNode.y + 1, currentNode.z)
        val blockCurrent = this.getCachedPathType(currentNode.x, currentNode.y, currentNode.z)

        // 计算步高
        val stepHeight =
            if (mob.getPathfindingMalus(blockAbove) >= 0.0f && blockCurrent != PathType.STICKY_HONEY) {
                Mth.floor(max(1.0, mob.maxUpStep().toDouble()))
            } else {
                0
            }
        ...
        if (isVerticalNeighborValid(belowNode, currentNode) && blockCurrent != PathType.TRAPDOOR) {
    ...
    private fun isVerticalNeighborValid(pNeighbor: Node?, pNode: Node): Boolean {
        return this.isNeighborValid(pNeighbor, pNode) && pNeighbor!!.type == PathType.LAVA
    }
