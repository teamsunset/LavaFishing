...
object BedrockLoader {
    ...
    val voidLayerDefinition: LayerDefinition = LayerDefinition.create(MeshDefinition(), 0, 0)
    val voidAnimation: AnimationDefinition = AnimationDefinition.Builder.withLength(0f).build()
    ...
    fun loadGeometry(resourcePath: String): LayerDefinition {
        try {
            val geo = this.javaClass.getResourceAsStream(resourcePath)?.reader().use {
            ...
            this.logger.error("Error loading model: ${e.message}")
            return this.voidLayerDefinition
        }
    }

    class AnimationManager(private val animations: Map<String, AnimationDefinition>) {
        operator fun get(name: String) = this.animations.getOrElse(name) { voidAnimation }
    }

    /** 解析 animation 文件为 AnimationDefinition 对象列表 */
    fun loadAnimations(resourcePath: String): AnimationManager {
        try {
            val animations = this.javaClass.getResourceAsStream(resourcePath)?.reader()?.use {
                        ...
                        val keyframes = timelineElem.asJsonObject.entrySet()
                            ...
                            .map { (timeStr, vecElem) ->
                                ...
                                val keyVec = when (targetEnum) {
                                    ...
                                    AnimationChannel.Targets.POSITION -> KeyframeAnimations.posVec(
                                        vec[0],
                                        vec[1],
                                        vec[2]
                                    )
            ...
            }.run { AnimationManager(this) }
        } catch (e: Exception) {
            this.logger.error("Error loading animations: ${e.message}")
            return AnimationManager(emptyMap())
        }
    }
}
