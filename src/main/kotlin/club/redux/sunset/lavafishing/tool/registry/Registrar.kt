...
import club.redux.sunset.lavafishing.util.UtilRegister.registerKt
...
abstract class Registrar<I>(key: ResourceKey<out Registry<I>>, modId: String) {
    constructor(reg: Registry<I>, modId: String) : this(reg.key(), modId)

    private val deferredRegister = UtilRegister.create(key, modId)
    fun attach(bus: IEventBus) = deferredRegister.register(bus)
    open fun <T : I> register(name: String, supplier: () -> T) = this.deferredRegister.registerKt(name, supplier)
