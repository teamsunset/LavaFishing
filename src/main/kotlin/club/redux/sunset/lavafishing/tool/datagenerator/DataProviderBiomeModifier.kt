...
import club.redux.sunset.lavafishing.util.UtilJson.toJsonArray
...
open class DataProviderBiomeModifier(
    ...
    private val pathProvider = output.createPathProvider(PackOutput.Target.DATA_PACK, "neoforge/biome_modifier")
    private val modifiers = mutableMapOf<String, Modifier>()

    fun addModifier(name: String, modifier: Modifier) = this.let { this.modifiers[name] = modifier }
    ...
    data class ModifierAddSpawns(
        ...
        override fun toJson(): JsonElement {
            return (super.toJson() as JsonObject).apply {
                add("biomes", this@ModifierAddSpawns.biomes)
                add("spawners", this@ModifierAddSpawns.spawners.toJsonArray())
            }
        }
