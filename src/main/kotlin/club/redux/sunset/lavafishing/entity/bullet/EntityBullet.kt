...
open class EntityBullet(
    ...
    init {
        this.setBaseDamage(this.calculateBaseDamage())
    }

    open fun calculateBaseDamage(): Double = 0.5 * this.tier.attackDamageBonus

    fun setFiringWeapon(stack: ItemStack, pierceLevel: Int) {
        this.firedFromWeapon = stack.copy()
        this.pierceLevel = pierceLevel.toByte()
    }

    // set to public
    public override fun setPickupItemStack(stack: ItemStack) = super.setPickupItemStack(stack)
    ...
    open fun attachEnchantmentEffects(stack: ItemStack) = Unit

    protected fun copyBulletPropertiesTo(target: EntityBullet) {
        target.owner = this.owner
        target.setBaseDamage(this.baseDamage)
        target.remainingFireTicks = this.remainingFireTicks
        target.setWaterInertia(this.getWaterInertia())

        this.weaponItem?.let { target.setFiringWeapon(it, this.pierceLevel.toInt()) }
    }
    ...
    override fun getDefaultHitGroundSoundEvent(): SoundEvent = SoundEvents.MUD_HIT
...
//            ForgeRegistries.SOUND_EVENTS.getValue(additionalData.readResourceLocation()) ?: SoundEvents.EMPTY
        this.waterInertia = buffer.readFloat()
    }
}
