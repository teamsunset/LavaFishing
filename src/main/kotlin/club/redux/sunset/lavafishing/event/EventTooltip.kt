...
object EventTooltip {
    private fun getTooltips(
        ...
        while (Language.getInstance().has(indexKey())) {
            tooltips += Component.translatable(indexKey())
                .withStyle(style)
                .also(processor)
                .also { if (!isShiftDown) it.appendShift() }
            index++
    ...
    fun onItemTooltip(event: ItemTooltipEvent) {
        ...
        val tooltipPath = "${BuildConstants.MOD_ID}.${itemLocation.path}.tooltip"
        ...
        val tooltips = getTooltips(tooltipPath, Style.EMPTY.applyFormats(color)) {
            ItemPromethiumBullet.appendTooltipCount(
                it,
                event.itemStack,
                "${BuildConstants.MOD_ID}.${itemLocation.path}.tooltip.title"
            )
        }
